...
| 1.3.53.1.13 | 2025/06/11 | 增加1.字符转译开关 2.自定义转译字符设置 3.ITF-25 将ITF-14转换为EAN13的开启/关闭 |
...
示例代码：
```
XcBarcodeScanner.needModifyGsCharacter(true);  // 开启
XcBarcodeScanner.needModifyGsCharacter(false); // 关闭
```

## 设置GS1 FNC1（0x1D）转译字符
可以通过接口设置GS1 FNC1（0x1D）转译字符（只能是单字符）
...
示例代码：
```
XcBarcodeScanner.escapeSingleCharacterSettings("a"); 替换为小写a
XcBarcodeScanner.escapeSingleCharacterSettings("A"); 替换为大写A
```

## 设置支持自定义转译字符
可以通过接口设置支持优先使用自定义转译字符
...
示例代码：
```
XcBarcodeScanner.customEscapeCharacters(true);  // 开启
XcBarcodeScanner.customEscapeCharacters(false); // 关闭
```

## 设置GS1 FNC1（0x1D）转译字符自定义
可以通过接口设置支持自定义的GS1 FNC1（0x1D）字符（最多支持五位）
...
示例代码：
```
XcBarcodeScanner.customConversionCharacters("abcdef"); 替换为abcde
